...
if pd.notna(data_min_base) and pd.notna(data_max_base):
    datas_selecionadas = st.sidebar.date_input(
        ...
        value=(data_min_base.date(), data_max_base.date()),
...
def processar_ambiente(df_input, termo_busca_tarefa, n_top_sugestao, nome_ambiente):
    """
...
with aba_separacao:
    processar_ambiente(df_filtrado, termo_busca_tarefa="SEPARA", n_top_sugestao=8, nome_ambiente="Separação")

with aba_conferencia:
    processar_ambiente(df_filtrado, termo_busca_tarefa="CONF", n_top_sugestao=2, nome_ambiente="Conferência")
